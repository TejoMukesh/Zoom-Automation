    ...
    if day_of_week in class_times.keys():
        ...
        for i in class_times[day_of_week]:
            if time.strptime(f'{today_date} {i[0]}','%Y-%m-%d %H %M') > time.localtime():
                x.enterabs((time.mktime(time.strptime(f'{today_date} {i[0]}',"%Y-%m-%d %H %M"))),0,webbrowser.open,kwargs={'url':i[1]})
                #to rejoin after 40 mins
                x.enterabs((time.mktime(time.strptime(f'{today_date} {get_new_time(i[0])}',"%Y-%m-%d %H %M"))),0,webbrowser.open,kwargs={'url':i[1]})
            else:
                pass
